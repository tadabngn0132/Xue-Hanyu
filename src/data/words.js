export const NEW_WORDS = [
  {
    id: 1,
    word: "你",
    pinyin: "nǐ",
    english: "you",
    vietnamese: "bạn",
    example: "你好！很高兴见到你。 (Hello! Nice to meet you.)",
    lesson: 1,
  },
  {
    id: 2,
    word: "好",
    pinyin: "hǎo",
    english: "good",
    vietnamese: "tốt",
    example: "今天天气很好。 (The weather is nice today.)",
    lesson: 1,
  },
  {
    id: 3,
    word: "我",
    pinyin: "wǒ",
    english: "I",
    vietnamese: "tôi",
    example: "我是学生。 (I am a student.)",
    lesson: 1,
  },
  {
    id: 4,
    word: "叫",
    pinyin: "jiào",
    english: "to be called",
    vietnamese: "được gọi là",
    example: "我叫小明。 (My name is Xiaoming.)",
    lesson: 1,
  },
  {
    id: 5,
    word: "是",
    pinyin: "shì",
    english: "to be",
    vietnamese: "là",
    example: "他是老师。 (He is a teacher.)",
    lesson: 1,
  },
  {
    id: 6,
    word: "人",
    pinyin: "rén",
    english: "person",
    vietnamese: "người",
    example: "他是我的朋友。 (He is my friend.)",
    lesson: 1,
  },
  {
    id: 7,
    word: "吗",
    pinyin: "ma",
    english: "吗 (question particle)",
    vietnamese: "à",
    example: "你是学生吗？ (Are you a student?)",
    lesson: 1,
  },
  {
    id: 8,
    word: "他",
    pinyin: "tā",
    english: "he/him",
    vietnamese: "anh ta",
    example: "他是我的兄弟。 (He is my brother.)",
    lesson: 1,
  },
  {
    id: 10,
    word: "不",
    pinyin: "bù",
    english: "not",
    vietnamese: "không",
    example: "我不想去。 (I don't want to go.)",
    lesson: 1,
  },
  {
    id: 11,
    word: "日本",
    pinyin: "Rìběn",
    english: "Japan",
    vietnamese: "Nhật Bản",
    example: "我是日本人。 (I am Japanese.)",
    lesson: 1,
  },
  {
    id: 12,
    word: "法国",
    pinyin: "Fǎguó",
    english: "France",
    vietnamese: "Pháp",
    example: "我是法国人。 (I am French.)",
    lesson: 1,
  },
  {
    id: 13,
    word: "加拿大",
    pinyin: "Jiānádá",
    english: "Canada",
    vietnamese: "Canada",
    example: "我是加拿大人。 (I am Canadian.)",
    lesson: 1,
  },
  {
    id: 14,
    word: "们",
    pinyin: "men",
    english: "we/us (plural marker)",
    vietnamese: "chúng ta",
    example: "我们是朋友。 (We are friends.)",
    lesson: 1,
  },
  {
    id: 15,
    word: "我们",
    pinyin: "wǒmen",
    english: "we/us",
    vietnamese: "chúng ta",
    example: "我们是朋友。 (We are friends.)",
    lesson: 1,
  },
  {
    id: 16,
    word: "你们",
    pinyin: "nǐmen",
    english: "you (plural)",
    vietnamese: "các bạn",
    example: "你们是学生。 (You are students.)",
    lesson: 1,
  },
  {
    id: 17,
    word: "他们",
    pinyin: "tāmen",
    english: "they/them (plural)",
    vietnamese: "họ",
    example: "他们是我的朋友。 (They are my friends.)",
    lesson: 1,
  },
  {
    id: 18,
    word: "她们",
    pinyin: "tāmen",
    english: "they/them (plural, feminine)",
    vietnamese: "họ (nữ)",
    example: "她们是我的朋友。 (They are my friends.)",
    lesson: 1,
  },
  {
    id: 19,
    word: "老师",
    pinyin: "lǎoshī",
    english: "teacher",
    vietnamese: "giáo viên",
    example: "他是我的老师。 (He is my teacher.)",
    lesson: 1,
  },
  {
    id: 20,
    word: "您",
    pinyin: "nín",
    english: "you (polite)",
    vietnamese: "ngài",
    example: "您好！ (Hello! - polite form)",
    lesson: 1,
  },
  {
    id: 21,
    word: "学生",
    pinyin: "xuésheng",
    english: "student",
    vietnamese: "học sinh",
    example: "他是学生。 (He is a student.)",
    lesson: 1,
  },
  {
    id: 22,
    word: "她",
    pinyin: "tā",
    english: "she/her",
    vietnamese: "cô ấy",
    example: "她是我的朋友。 (She is my friend.)",
    lesson: 1,
  },
  {
    id: 23,
    word: "也",
    pinyin: "yě",
    english: "also",
    vietnamese: "cũng",
    example: "我也想去。 (I want to go too.)",
    lesson: 1,
  },
  {
    id: 24,
    word: "中国",
    pinyin: "Zhōngguó",
    english: "China",
    vietnamese: "Trung Quốc",
    example: "我是中国人。 (I am Chinese.)",
    lesson: 1,
  },
  {
    id: 25,
    word: "这",
    pinyin: "zhè",
    english: "this",
    vietnamese: "đây",
    example: "这是书。 (This is a book.)",
    lesson: 2,
  },
  {
    id: 26,
    word: "的",
    pinyin: "de",
    english: "的 (possessive particle)",
    vietnamese: "của",
    example: "这是我的书。 (This is my book.)",
    lesson: 2,
  },
  {
    id: 27,
    word: "书",
    pinyin: "shū",
    english: "book",
    vietnamese: "sách",
    example: "这是书。 (This is a book.)",
    lesson: 2,
  },
  {
    id: 28,
    word: "词典",
    pinyin: "cídiǎn",
    english: "dictionary",
    vietnamese: "từ điển",
    example: "这是词典。 (This is a dictionary.)",
    lesson: 2,
  },
  {
    id: 29,
    word: "地图",
    pinyin: "dìtú",
    english: "map",
    vietnamese: "bản đồ",
    example: "这是地图。 (This is a map.)",
    lesson: 2,
  },
  {
    id: 30,
    word: "本子",
    pinyin: "běnzi",
    english: "notebook",
    vietnamese: "vở",
    example: "这是笔记本。 (This is a notebook.)",
    lesson: 2,
  },
  {
    id: 31,
    word: "笔",
    pinyin: "bǐ",
    english: "pen",
    vietnamese: "bút",
    example: "这是笔。 (This is a pen.)",
    lesson: 2,
  },
  {
    id: 32,
    word: "有",
    pinyin: "yǒu",
    english: "to have",
    vietnamese: "có",
    example: "我有书。 (I have a book.)",
    lesson: 2,
  },
  {
    id: 33,
    word: "没有",
    pinyin: "méiyǒu",
    english: "do not have",
    vietnamese: "không có",
    example: "我没有书。 (I do not have a book.)",
    lesson: 2,
  },
  {
    id: 34,
    word: "支",
    pinyin: "zhī",
    english: "measure word for pen",
    vietnamese: "cây (đơn vị đếm cho bút)",
    example: "我有一支笔。 (I have a pen.)",
    lesson: 2,
  },
  {
    id: 35,
    word: "个",
    pinyin: "ge",
    english: "measure word for general items",
    vietnamese: "cái (đơn vị đếm chung)",
    example: "我有一个苹果。 (I have an apple.)",
    lesson: 2,
  },
  {
    id: 36,
    word: "两",
    pinyin: "liǎng",
    english: "two",
    vietnamese: "hai",
    example: "我有两本书。 (I have two books.)",
    lesson: 2,
  },
  {
    id: 37,
    word: "本",
    pinyin: "běn",
    english: "measure word for books",
    vietnamese: "cuốn (đơn vị đếm cho sách)",
    example: "我有一本书。 (I have a book.)",
    lesson: 2,
  },
  {
    id: 38,
    word: "张",
    pinyin: "zhāng",
    english: "measure word for papers, tickets, etc.",
    vietnamese: "chiếc (đơn vị đếm cho giấy, vé, v.v.)",
    example: "我有一张纸。 (I have a piece of paper.)",
    lesson: 2,
  },
  {
    id: 39,
    word: "什么",
    pinyin: "shénme",
    english: "what",
    vietnamese: "gì",
    example: "什么？ (What?)",
    lesson: 3,
  },
  {
    id: 40,
    word: "家",
    pinyin: "jiā",
    english: "home/family",
    vietnamese: "nhà/gia đình",
    example: "我爱我的家。 (I love my family.)",
    lesson: 3,
  },
  {
    id: 41,
    word: "照片",
    pinyin: "zhàopiàn",
    english: "photo",
    vietnamese: "ảnh",
    example: "这是照片。 (This is a photo.)",
    lesson: 3,
  },
  {
    id: 42,
    word: "几",
    pinyin: "jǐ",
    english: "how many",
    vietnamese: "mấy (dùng để hỏi số lượng dưới 10)",
    example: "几本书？ (How many books?)",
    lesson: 3,
  },
  {
    id: 43,
    word: "口",
    pinyin: "kǒu",
    english: "measure word for family members",
    vietnamese: "khẩu (đơn vị đếm cho thành viên gia đình)",
    example: "我家有五口人。 (There are five people in my family.)",
    lesson: 3,
  },
  {
    id: 44,
    word: "谁",
    pinyin: "shéi/shuí",
    english: "who",
    vietnamese: "ai",
    example: "谁是你的朋友？ (Who is your friend?)",
    lesson: 3,
  },
  {
    id: 45,
    word: "哥哥",
    pinyin: "gēge",
    english: "older brother",
    vietnamese: "anh trai",
    example: "那是我的哥哥。 (That is my older brother.)",
    lesson: 3,
  },
  {
    id: 46,
    word: "爸爸",
    pinyin: "bàba",
    english: "father",
    vietnamese: "ba",
    example: "那是我的爸爸。 (That is my father.)",
    lesson: 3,
  },
  {
    id: 47,
    word: "妈妈",
    pinyin: "māma",
    english: "mother",
    vietnamese: "mẹ",
    example: "那是我的妈妈。 (That is my mother.)",
    lesson: 3,
  },
  {
    id: 48,
    word: "姐姐",
    pinyin: "jiějie",
    english: "older sister",
    vietnamese: "chị gái",
    example: "那是我的姐姐。 (That is my older sister.)",
    lesson: 3,
  },
  {
    id: 49,
    word: "和",
    pinyin: "hé",
    english: "and",
    vietnamese: "và",
    example: "我和我的朋友。 (Me and my friend.)",
    lesson: 3,
  },
  {
    id: 50,
    word: "做",
    pinyin: "zuò",
    english: "to do, to make",
    vietnamese: "làm",
    example: "我喜欢做饭。 (I like to cook.)",
    lesson: 3,
  },
  {
    id: 51,
    word: "工作",
    pinyin: "gōngzuò",
    english: "work",
    vietnamese: "làm việc",
    example: "我在工作。 (I am working.)",
    lesson: 3,
  },
  {
    id: 52,
    word: "妹妹",
    pinyin: "mèimei",
    english: "younger sister",
    vietnamese: "em gái",
    example: "那是我的妹妹。 (That is my younger sister.)",
    lesson: 3,
  },
  {
    id: 53,
    word: "大夫",
    pinyin: "daifu",
    english: "doctor",
    vietnamese: "bác sĩ",
    example: "那是我的大夫。 (That is my doctor.)",
    lesson: 3,
  },
  {
    id: 54,
    word: "全",
    pinyin: "quán",
    english: "whole, all",
    vietnamese: "toàn bộ",
    example: "全世界。 (The whole world.)",
    lesson: 3,
  },
  {
    id: 55,
    word: "弟弟",
    pinyin: "dìdi",
    english: "younger brother",
    vietnamese: "em trai",
    example: "那是我的弟弟。 (That is my younger brother.)",
    lesson: 3,
  },
  {
    id: 56,
    word: "班",
    pinyin: "bān",
    english: "class",
    vietnamese: "lớp học",
    example: "我们在上课。 (We are in class.)",
    lesson: 4,
  },
  {
    id: 57,
    word: "多少",
    pinyin: "duōshǎo",
    english: "how many/how much",
    vietnamese: "bao nhiêu",
    example: "多少人？ (How many people?)",
    lesson: 4,
  },
  {
    id: 58,
    word: "学习",
    pinyin: "xuéxí",
    english: "to study",
    vietnamese: "học",
    example: "我在学习。 (I am studying.)",
    lesson: 4,
  },
  {
    id: 59,
    word: "学",
    pinyin: "xué",
    english: "to study",
    vietnamese: "học",
    example: "我在学习。 (I am studying.)",
    lesson: 4,
  },
  {
    id: 60,
    word: "汉语",
    pinyin: "Hànyǔ",
    english: "Chinese language",
    vietnamese: "Ngôn ngữ Trung Quốc",
    example: "我想学汉语。 (I want to learn Chinese.)",
    lesson: 4,
  },
  {
    id: 61,
    word: "语",
    pinyin: "yǔ",
    english: "language",
    vietnamese: "ngôn ngữ",
    example: "汉语是我们的母语。 (Chinese is our native language.)",
    lesson: 4,
  },
  {
    id: 62,
    word: "英语",
    pinyin: "Yīngyǔ",
    english: "English language",
    vietnamese: "Ngôn ngữ Anh",
    example: "我想学英语。 (I want to learn English.)",
    lesson: 4,
  },
  {
    id: 63,
    word: "难",
    pinyin: "nan",
    english: "difficult",
    vietnamese: "khó",
    example: "汉语很难。 (Chinese is difficult.)",
    lesson: 4,
  },
  {
    id: 64,
    word: "同学",
    pinyin: "tóngxué",
    english: "classmate",
    vietnamese: "bạn học",
    example: "他是我的同学。 (He is my classmate.)",
    lesson: 4,
  },
  {
    id: 65,
    word: "男生",
    pinyin: "nánshēng",
    english: "boy",
    vietnamese: "nam sinh",
    example: "他是我的男生朋友。 (He is my male friend.)",
    lesson: 4,
  },
  {
    id: 66,
    word: "男",
    pinyin: "nán",
    english: "male",
    vietnamese: "nam",
    example: "他是男生。 (He is a boy.)",
    lesson: 4,
  },
  {
    id: 67,
    word: "女生",
    pinyin: "nǚshēng",
    english: "girl",
    vietnamese: "nữ sinh",
    example: "她是我的女生朋友。 (She is my female friend.)",
    lesson: 4,
  },
  {
    id: 68,
    word: "女",
    pinyin: "nǚ",
    english: "female",
    vietnamese: "nữ",
    example: "她是女生。 (She is a girl.)",
    lesson: 4,
  },
  {
    id: 69,
    word: "都",
    pinyin: "dōu",
    english: "all, both",
    vietnamese: "đều",
    example: "我们都是学生。 (We are all students.)",
    lesson: 4,
  },
  {
    id: 70,
    word: "请",
    pinyin: "qǐng",
    english: "please (term of respect)",
    vietnamese: "xin/mời/làm ơn",
    example: "请坐。 (Please sit down.)",
    lesson: 4,
  },
  {
    id: 71,
    word: "进",
    pinyin: "jìn",
    english: "to enter",
    vietnamese: "vào",
    example: "请进来。 (Please come in.)",
    lesson: 4,
  },
  {
    id: 72,
    word: "外国",
    pinyin: "wàiguó",
    english: "foreign",
    vietnamese: "nước ngoài",
    example: "外国朋友。 (Foreign friend.)",
    lesson: 4,
  },
  {
    id: 73,
    word: "朋友",
    pinyin: "péngyǒu",
    english: "friend",
    vietnamese: "bạn bè",
    example: "他是我的朋友。 (He is my friend.)",
    lesson: 4,
  },
  {
    id: 74,
    word: "名字",
    pinyin: "míngzì",
    english: "name",
    vietnamese: "tên",
    example: "你叫什么名字？ (What is your name?)",
    lesson: 4,
  },
  {
    id: 75,
    word: "哪",
    pinyin: "nǎ/něi",
    english: "which",
    vietnamese: "nào",
    example: "哪一个是你的书？ (Which one is your book?)",
    lesson: 4,
  },
  {
    id: 76,
    word: "国",
    pinyin: "guó",
    english: "country",
    vietnamese: "quốc gia",
    example: "你是哪国人？ (Which country are you from?)",
    lesson: 4,
  },
  {
    id: 77,
    word: "一",
    pinyin: "yī",
    english: "one",
    vietnamese: "một",
    example: "我有一本书。 (I have one book.)",
    lesson: 4,
  },
  {
    id: 78,
    word: "二",
    pinyin: "èr",
    english: "two",
    vietnamese: "hai",
    example: "我有两本书。 (I have two books.)",
    lesson: 4,
  },
  {
    id: 79,
    word: "三",
    pinyin: "sān",
    english: "three",
    vietnamese: "ba",
    example: "我有三本书。 (I have three books.)",
    lesson: 4,
  },
  {
    id: 80,
    word: "四",
    pinyin: "sì",
    english: "four",
    vietnamese: "bốn",
    example: "我有四本书。 (I have four books.)",
    lesson: 4,
  },
  {
    id: 81,
    word: "五",
    pinyin: "wǔ",
    english: "five",
    vietnamese: "năm",
    example: "我有五本书。 (I have five books.)",
    lesson: 4,
  },
  {
    id: 82,
    word: "六",
    pinyin: "liù",
    english: "six",
    vietnamese: "sáu",
    example: "我有六本书。 (I have six books.)",
    lesson: 4,
  },
  {
    id: 83,
    word: "七",
    pinyin: "qī",
    english: "seven",
    vietnamese: "bảy",
    example: "我有七本书。 (I have seven books.)",
    lesson: 4,
  },
  {
    id: 84,
    word: "八",
    pinyin: "bā",
    english: "eight",
    vietnamese: "tám",
    example: "我有八本书。 (I have eight books.)",
    lesson: 4,
  },
  {
    id: 85,
    word: "九",
    pinyin: "jiǔ",
    english: "nine",
    vietnamese: "chín",
    example: "我有九本书。 (I have nine books.)",
    lesson: 4,
  },
  {
    id: 86,
    word: "十",
    pinyin: "shí",
    english: "ten",
    vietnamese: "mười",
    example: "我有十本书。 (I have ten books.)",
    lesson: 4,
  },
  {
    id: 87,
    word: "今天",
    pinyin: "jīntiān",
    english: "today",
    vietnamese: "hôm nay",
    example: "今天天气很好。 (The weather is very nice today.)",
    lesson: 5,
  },
  {
    id: 88,
    word: "天",
    pinyin: "tiān",
    english: "day",
    vietnamese: "ngày",
    example: "今天是星期一。 (Today is Monday.)",
    lesson: 5,
  },
  {
    id: 89,
    word: "昨天",
    pinyin: "zuótiān",
    english: "yesterday",
    vietnamese: "hôm qua",
    example: "昨天天气很冷。 (The weather was very cold yesterday.)",
    lesson: 5,
  },
  {
    id: 90,
    word: "明天",
    pinyin: "míngtiān",
    english: "tomorrow",
    vietnamese: "ngày mai",
    example: "明天天气会更好。 (The weather will be better tomorrow.)",
    lesson: 5,
  },
  {
    id: 91,
    word: "生日",
    pinyin: "shēngrì",
    english: "birthday",
    vietnamese: "sinh nhật",
    example: "今天是我的生日。 (Today is my birthday.)",
    lesson: 5,
  },
  {
    id: 92,
    word: "祝",
    pinyin: "zhù",
    english: "to wish someone a happy birthday",
    vietnamese: "chúc mừng sinh nhật",
    example: "祝你生日快乐！ (Happy birthday to you!)",
    lesson: 5,
  },
  {
    id: 93,
    word: "快乐",
    pinyin: "kuàilè",
    english: "happy",
    vietnamese: "vui vẻ",
    example: "祝你生日快乐！ (Happy birthday to you!)",
    lesson: 5,
  },
  {
    id: 94,
    word: "谢谢",
    pinyin: "xièxie",
    english: "thank you",
    vietnamese: "cảm ơn",
    example: "谢谢你！ (Thank you!)",
    lesson: 5,
  },
  {
    id: 95,
    word: "不谢",
    pinyin: "bù xiè",
    english: "you're welcome",
    vietnamese: "không có gì",
    example: "不谢！ (You're welcome!)",
    lesson: 5,
  },
  {
    id: 96,
    word: "礼物",
    pinyin: "lǐwù",
    english: "gift",
    vietnamese: "quà tặng",
    example: "这是给你的礼物。 (This is a gift for you.)",
    lesson: 5,
  },
  {
    id: 97,
    word: "对",
    pinyin: "duì",
    english: "yes, all right, correct",
    vietnamese: "vâng, đúng rồi, chính xác",
    example: "对，你说得对。 (Yes, you're right.)",
    lesson: 5,
  },
  {
    id: 98,
    word: "月",
    pinyin: "yuè",
    english: "month",
    vietnamese: "tháng",
    example: "今天是三月。 (Today is March.)",
    lesson: 5,
  },
  {
    id: 99,
    word: "号/日",
    pinyin: "hào/rì",
    english: "day of the month",
    vietnamese: "ngày (trong tháng)",
    example: "今天是三月十五号。 (Today is March 15th.)",
    lesson: 5,
  },
  {
    id: 100,
    word: "星期",
    pinyin: "xīngqī",
    english: "week",
    vietnamese: "tuần",
    example: "今天是星期五。 (Today is Friday.)",
    lesson: 5,
  },
  {
    id: 101,
    word: "星期一",
    pinyin: "xīngqīyī",
    english: "Monday",
    vietnamese: "thứ hai",
    example: "今天是星期一。 (Today is Monday.)",
    lesson: 5,
  },
  {
    id: 102,
    word: "星期二",
    pinyin: "xīngqī'èr",
    english: "Tuesday",
    vietnamese: "thứ ba",
    example: "今天是星期二。 (Today is Tuesday.)",
    lesson: 5,
  },
  {
    id: 103,
    word: "星期三",
    pinyin: "xīngqīsān",
    english: "Wednesday",
    vietnamese: "thứ tư",
    example: "今天是星期三。 (Today is Wednesday.)",
    lesson: 5,
  },
  {
    id: 104,
    word: "星期四",
    pinyin: "xīngqīsì",
    english: "Thursday",
    vietnamese: "thứ năm",
    example: "今天是星期四。 (Today is Thursday.)",
    lesson: 5,
  },
  {
    id: 105,
    word: "星期五",
    pinyin: "xīngqīwǔ",
    english: "Friday",
    vietnamese: "thứ sáu",
    example: "今天是星期五。 (Today is Friday.)",
    lesson: 5,
  },
  {
    id: 106,
    word: "星期六",
    pinyin: "xīngqīliù",
    english: "Saturday",
    vietnamese: "thứ bảy",
    example: "今天是星期六。 (Today is Saturday.)",
    lesson: 5,
  },
  {
    id: 107,
    word: "星期日/天",
    pinyin: "xīngqīrì/tiān",
    english: "Sunday",
    vietnamese: "chủ nhật",
    example: "今天是星期日。 (Today is Sunday.)",
    lesson: 5,
  },
  {
    id: 108,
    word: "送",
    pinyin: "sòng",
    english: "to give",
    vietnamese: "tặng",
    example: "我送你一个礼物。 (I give you a gift.)",
    lesson: 5,
  },
  {
    id: 109,
    word: "很",
    pinyin: "hěn",
    english: "very",
    vietnamese: "rất",
    example: "今天天气很好。 (The weather is very nice today.)",
    lesson: 5,
  },
  {
    id: 110,
    word: "高兴",
    pinyin: "gāoxìng",
    english: "happy",
    vietnamese: "vui mừng",
    example: "很高兴认识你！ (Nice to meet you!)",
    lesson: 5,
  },
  {
    id: 111,
    word: "元旦",
    pinyin: "Yuándàn",
    english: "New Year's Day",
    vietnamese: "Tết dương lịch",
    example: "元旦快乐！ (Happy New Year!)",
    lesson: 5,
  },
  {
    id: 112,
    word: "妇女节",
    pinyin: "Fùnǚ Jié",
    english: "Women's Day",
    vietnamese: "Ngày Phụ nữ",
    example: "妇女节快乐！ (Happy Women's Day!)",
    lesson: 5,
  },
  {
    id: 113,
    word: "劳动节",
    pinyin: "Láodòng Jié",
    english: "Labor Day",
    vietnamese: "Ngày Lao động",
    example: "劳动节快乐！ (Happy Labor Day!)",
    lesson: 5,
  },
  {
    id: 114,
    word: "青年节",
    pinyin: "Qīngnián Jié",
    english: "Youth Day",
    vietnamese: "Ngày Thanh niên",
    example: "青年节快乐！ (Happy Youth Day!)",
    lesson: 5,
  },
  {
    id: 115,
    word: "儿童节",
    pinyin: "Értóng Jié",
    english: "Children's Day",
    vietnamese: "Ngày Quốc tế Thiếu nhi",
    example: "儿童节快乐！ (Happy Children's Day!)",
    lesson: 5,
  },
  {
    id: 116,
    word: "教师节",
    pinyin: "Jiàoshī Jié",
    english: "Teacher's Day",
    vietnamese: "Ngày Nhà giáo",
    example: "教师节快乐！ (Happy Teacher's Day!)",
    lesson: 5,
  },
  {
    id: 117,
    word: "国庆节",
    pinyin: "Guóqìng Jié",
    english: "National Day",
    vietnamese: "Ngày Quốc khánh",
    example: "国庆节快乐！ (Happy National Day!)",
    lesson: 5,
  },
  {
    id: 118,
    word: "问",
    pinyin: "wèn",
    english: "to ask",
    vietnamese: "hỏi",
    example: "你叫什么名字？ (What is your name?)",
    lesson: 6,
  },
  {
    id: 119,
    word: "现在",
    pinyin: "xiànzài",
    english: "now",
    vietnamese: "bây giờ",
    example: "现在几点了？ (What time is it now?)",
    lesson: 6,
  },
  {
    id: 120,
    word: "点",
    pinyin: "diǎn",
    english: "o'clock",
    vietnamese: "giờ",
    example: "现在三点了。 (It is three o'clock now.)",
    lesson: 6,
  },
  {
    id: 121,
    word: "零",
    pinyin: "líng",
    english: "zero",
    vietnamese: "không",
    example: "现在是三点零五分。 (It is three zero five now.)",
    lesson: 6,
  },
  {
    id: 122,
    word: "分",
    pinyin: "fēn",
    english: "minute",
    vietnamese: "phút",
    example: "现在是三点十分。 (It is three ten now.)",
    lesson: 6,
  },
  {
    id: 123,
    word: "不用",
    pinyin: "bù yòng",
    english: "no need",
    vietnamese: "không cần",
    example: "不用谢！ (No need to thank me!)",
    lesson: 6,
  },
  {
    id: 124,
    word: "用",
    pinyin: "yòng",
    english: "to use",
    vietnamese: "dùng",
    example: "我用这支笔写字。 (I use this pen to write.)",
    lesson: 6,
  },
  {
    id: 125,
    word: "早上",
    pinyin: "zǎoshang",
    english: "morning",
    vietnamese: "sáng",
    example: "早上好！ (Good morning!)",
    lesson: 6,
  },
  {
    id: 126,
    word: "上午",
    pinyin: "shàngwǔ",
    english: "forenoon",
    vietnamese: "buổi sáng (trước 12 giờ)",
    example: "上午好！ (Good morning!)",
    lesson: 6,
  },
  {
    id: 127,
    word: "中午",
    pinyin: "zhōngwǔ",
    english: "noon",
    vietnamese: "trưa",
    example: "中午好！ (Good afternoon!)",
    lesson: 6,
  },
  {
    id: 128,
    word: "下午",
    pinyin: "xiàwǔ",
    english: "afternoon",
    vietnamese: "buổi chiều",
    example: "下午好！ (Good afternoon!)",
    lesson: 6,
  },
  {
    id: 129,
    word: "晚上",
    pinyin: "wǎnshang",
    english: "evening",
    vietnamese: "tối",
    example: "晚上好！ (Good evening!)",
    lesson: 6,
  },
  {
    id: 130,
    word: "床",
    pinyin: "chuáng",
    english: "bed",
    vietnamese: "giường",
    example: "我在床上看书。 (I am reading a book in bed.)",
    lesson: 6,
  },
  {
    id: 131,
    word: "刻",
    pinyin: "kè",
    english: "quarter (of an hour)",
    vietnamese: "một phần tư (của một giờ)",
    example: "现在是三点十分。 (It is three ten now.)",
    lesson: 6,
  },
  {
    id: 132,
    word: "睡觉",
    pinyin: "shuìjiào",
    english: "to sleep",
    vietnamese: "ngủ",
    example: "我要去睡觉了。 (I am going to sleep.)",
    lesson: 6,
  },
  {
    id: 133,
    word: "睡",
    pinyin: "shuì",
    english: "to sleep",
    vietnamese: "ngủ",
    example: "我要去睡觉了。 (I am going to sleep.)",
    lesson: 6,
  },
  {
    id: 134,
    word: "课",
    pinyin: "kè",
    english: "lesson, class",
    vietnamese: "bài học",
    example: "我们今天有三节课。 (We have three classes today.)",
    lesson: 6,
  },
  {
    id: 135,
    word: "上课",
    pinyin: "shàngkè",
    english: "to have class",
    vietnamese: "lên lớp",
    example: "我们上课了。 (We are having class.)",
    lesson: 6,
  },
  {
    id: 136,
    word: "上",
    pinyin: "shàng",
    english: "to go up, to attend (class)",
    vietnamese: "lên, tham dự (lớp học)",
    example: "我们上课了。 (We are having class.)",
    lesson: 6,
  },
  {
    id: 137,
    word: "时候",
    pinyin: "shíhou",
    english: "time, moment",
    vietnamese: "thời gian, lúc",
    example: "上课的时候请安静。 (Please be quiet during class.)",
    lesson: 6,
  },
  {
    id: 138,
    word: "下课",
    pinyin: "xiàkè",
    english: "to finish class",
    vietnamese: "kết thúc lớp",
    example: "下课了。 (The class is over.)",
    lesson: 6,
  },
  {
    id: 139,
    word: "下",
    pinyin: "xià",
    english: "to go down, to finish (class)",
    vietnamese: "xuống, kết thúc (lớp học)",
    example: "下课了。 (The class is over.)",
    lesson: 6,
  },
  {
    id: 140,
    word: "半",
    pinyin: "bàn",
    english: "half (of an hour)",
    vietnamese: "nửa (của một giờ)",
    example: "现在是三点半。 (It is three thirty now.)",
    lesson: 6,
  },
  {
    id: 141,
    word: "差",
    pinyin: "chà",
    english: "to be short of, to lack",
    vietnamese: "kém",
    example: "现在差五分四点。 (It is five minutes to four now.)",
    lesson: 6,
  },
  {
    id: 142,
    word: "银行",
    pinyin: "yínháng",
    english: "bank",
    vietnamese: "ngân hàng",
    example: "我去银行存钱。 (I am going to the bank to deposit money.)",
    lesson: 6,
  },
  {
    id: 143,
    word: "在",
    pinyin: "zài",
    english: "at, in",
    vietnamese: "tại",
    example: "我在家里。 (I am at home.)",
    lesson: 7,
  },
  {
    id: 144,
    word: "哪儿",
    pinyin: "nǎr",
    english: "where",
    vietnamese: "ở đâu",
    example: "哪儿有书店？ (Where is the bookstore?)",
    lesson: 7,
  },
  {
    id: 145,
    word: "东边",
    pinyin: "dōngbian",
    english: "east, east side",
    vietnamese: "đông, phía đông",
    example: "太阳从东方升起。 (The sun rises from the east.)",
    lesson: 7,
  },
  {
    id: 146,
    word: "东",
    pinyin: "dōng",
    english: "east",
    vietnamese: "đông",
    example: "太阳从东方升起。 (The sun rises from the east.)",
    lesson: 7,
  },
  {
    id: 147,
    word: "边",
    pinyin: "biān",
    english: "side",
    vietnamese: "bên",
    example:
      "东边和西边都有商店。 (There are stores on both the east side and the west side.)",
    lesson: 7,
  },
  {
    id: 148,
    word: "西边",
    pinyin: "xībian",
    english: "west, west side",
    vietnamese: "tây, phía tây",
    example: "太阳从西方落下。 (The sun sets in the west.)",
    lesson: 7,
  },
  {
    id: 149,
    word: "西",
    pinyin: "xī",
    english: "west",
    vietnamese: "tây",
    example: "太阳从西方落下。 (The sun sets in the west.)",
    lesson: 7,
  },
  {
    id: 150,
    word: "地方",
    pinyin: "dìfang",
    english: "place",
    vietnamese: "địa điểm",
    example: "这个地方很漂亮。 (This place is very beautiful.)",
    lesson: 7,
  },
  {
    id: 151,
    word: "座",
    pinyin: "zuò",
    english: "a measure word for buildings",
    vietnamese: "lượng từ cho các tòa nhà",
    example: "我住在一座大楼里。 (I live in a big building.)",
    lesson: 7,
  },
  {
    id: 152,
    word: "楼",
    pinyin: "lóu",
    english: "floor, story",
    vietnamese: "tầng",
    example: "我住在五楼。 (I live on the fifth floor.)",
    lesson: 7,
  },
  {
    id: 153,
    word: "书店",
    pinyin: "shūdiàn",
    english: "bookstore",
    vietnamese: "tiệm sách",
    example: "我去书店买书。 (I am going to the bookstore to buy books.)",
    lesson: 7,
  },
  {
    id: 154,
    word: "店",
    pinyin: "diàn",
    english: "store",
    vietnamese: "cửa hàng",
    example: "我去书店买书。 (I am going to the bookstore to buy books.)",
    lesson: 7,
  },
  {
    id: 155,
    word: "那",
    pinyin: "nà",
    english: "that",
    vietnamese: "đó",
    example: "那是我的书店。 (That is my bookstore.)",
    lesson: 7,
  },
  {
    id: 156,
    word: "不客气",
    pinyin: "bú kèqi",
    english: "you're welcome",
    vietnamese: "không có gì",
    example: "不客气！ (You're welcome!)",
    lesson: 7,
  },
  {
    id: 157,
    word: "客气",
    pinyin: "kèqi",
    english: "polite",
    vietnamese: "lịch sự",
    example: "不客气！ (You're welcome!)",
    lesson: 7,
  },
  {
    id: 158,
    word: "学校",
    pinyin: "xuéxiào",
    english: "school",
    vietnamese: "trường học",
    example: "我在学校上课。 (I am attending class at school.)",
    lesson: 7,
  },
  {
    id: 159,
    word: "宿舍",
    pinyin: "sushe",
    english: "dormitory",
    vietnamese: "ký túc xá",
    example: "我住在学校的宿舍里。 (I live in the school's dormitory.)",
    lesson: 7,
  },
  {
    id: 160,
    word: "北边",
    pinyin: "běibian",
    english: "north, north side",
    vietnamese: "bắc, phía bắc",
    example: "太阳从北方升起。 (The sun rises from the north.)",
    lesson: 7,
  },
  {
    id: 161,
    word: "北",
    pinyin: "běi",
    english: "north",
    vietnamese: "bắc",
    example: "太阳从北方升起。 (The sun rises from the north.)",
    lesson: 7,
  },
  {
    id: 162,
    word: "南边",
    pinyin: "nánbian",
    english: "south, south side",
    vietnamese: "nam, phía nam",
    example: "太阳从南方落下。 (The sun sets in the south.)",
    lesson: 7,
  },
  {
    id: 163,
    word: "南",
    pinyin: "nán",
    english: "south",
    vietnamese: "nam",
    example: "太阳从南方落下。 (The sun sets in the south.)",
    lesson: 7,
  },
  {
    id: 164,
    word: "号",
    pinyin: "hào",
    english: "number",
    vietnamese: "số",
    example: "这是我的号码。 (This is my phone number.)",
    lesson: 7,
  },
  {
    id: 165,
    word: "前面/前边",
    pinyin: "qiánmiàn/qiánbiān",
    english: "front",
    vietnamese: "phía trước",
    example: "我在前面等你。 (I am waiting for you in front.)",
    lesson: 7,
  },
  {
    id: 166,
    word: "前",
    pinyin: "qián",
    english: "front",
    vietnamese: "trước",
    example: "我在前面等你。 (I am waiting for you in front.)",
    lesson: 7,
  },
  {
    id: 167,
    word: "后面/后边",
    pinyin: "hòumiàn/hòubiān",
    english: "back",
    vietnamese: "phía sau",
    example: "我在后面等你。 (I am waiting for you in back.)",
    lesson: 7,
  },
  {
    id: 168,
    word: "后",
    pinyin: "hòu",
    english: "back",
    vietnamese: "sau",
    example: "我在后面等你。 (I am waiting for you in back.)",
    lesson: 7,
  },
  {
    id: 169,
    word: "房间",
    pinyin: "fángjiān",
    english: "room",
    vietnamese: "phòng",
    example: "我在房间里。 (I am in the room.)",
    lesson: 8,
  },
  {
    id: 170,
    word: "里",
    pinyin: "lǐ",
    english: "inside",
    vietnamese: "bên trong",
    example: "我在房间里。 (I am in the room.)",
    lesson: 8,
  },
  {
    id: 171,
    word: "里面/里边",
    pinyin: "lǐmiàn/lǐbiān",
    english: "inside",
    vietnamese: "bên trong",
    example: "书在书里面。 (The book is inside the box.)",
    lesson: 8,
  },
  {
    id: 172,
    word: "外",
    pinyin: "wài",
    english: "outside",
    vietnamese: "bên ngoài",
    example: "书在箱子外面。 (The book is outside the box.)",
    lesson: 8,
  },
  {
    id: 173,
    word: "外面/外边",
    pinyin: "wàimiàn/wàibiān",
    english: "outside",
    vietnamese: "bên ngoài",
    example: "书在箱子外面。 (The book is outside the box.)",
    lesson: 8,
  },
  {
    id: 174,
    word: "桌子",
    pinyin: "zhuōzi",
    english: "table, desk",
    vietnamese: "bàn, giá",
    example: "我在桌子上。 (I am at the table.)",
    lesson: 8,
  },
  {
    id: 175,
    word: "把",
    pinyin: "bǎ",
    english: "a measure word for things with handles",
    vietnamese: "lượng từ cho các vật có tay cầm",
    example: "我有一把椅子。 (I have a chair.)",
    lesson: 8,
  },
  {
    id: 176,
    word: "椅子",
    pinyin: "yǐzi",
    english: "chair",
    vietnamese: "ghế",
    example: "我有一把椅子。 (I have a chair.)",
    lesson: 8,
  },
  {
    id: 177,
    word: "书架",
    pinyin: "shūjià",
    english: "bookshelf",
    vietnamese: "kệ sách",
    example: "书在书架上。 (The book is on the bookshelf.)",
    lesson: 8,
  },
  {
    id: 178,
    word: "柜子",
    pinyin: "guizi",
    english: "wardrobe",
    vietnamese: "tủ",
    example: "书在柜子里。 (The book is in the wardrobe.)",
    lesson: 8,
  },
  {
    id: 179,
    word: "墙",
    pinyin: "qiáng",
    english: "wall",
    vietnamese: "tường",
    example: "书在墙上。 (The book is on the wall.)",
    lesson: 8,
  },
  {
    id: 180,
    word: "上",
    pinyin: "shàng",
    english: "above, on top of",
    vietnamese: "trên",
    example: "书在桌子上。 (The book is on the table.)",
    lesson: 8,
  },
  {
    id: 181,
    word: "上面/上边",
    pinyin: "shàngmiàn/shàngbiān",
    english: "above, on top of",
    vietnamese: "phía trên",
    example: "书在桌子上。 (The book is on the table.)",
    lesson: 8,
  },
  {
    id: 182,
    word: "左边/左面",
    pinyin: "zuǒbiān/zuǒmiàn",
    english: "left",
    vietnamese: "phía trái",
    example: "我在左边等你。 (I am waiting for you on the left.)",
    lesson: 8,
  },
  {
    id: 183,
    word: "左",
    pinyin: "zuǒ",
    english: "left",
    vietnamese: "trái",
    example: "我在左边等你。 (I am waiting for you on the left.)",
    lesson: 8,
  },
  {
    id: 184,
    word: "右边/右面",
    pinyin: "yòubiān/yòumiàn",
    english: "right",
    vietnamese: "phía phải",
    example: "我在右边等你。 (I am waiting for you on the right.)",
    lesson: 8,
  },
  {
    id: 185,
    word: "右",
    pinyin: "yòu",
    english: "right",
    vietnamese: "phải",
    example: "我在右边等你。 (I am waiting for you on the right.)",
    lesson: 8,
  },
  {
    id: 186,
    word: "旁边/旁面",
    pinyin: "pángbiān/pángmiàn",
    english: "beside, next to",
    vietnamese: "bên cạnh",
    example: "我在旁边等你。 (I am waiting for you beside the table.)",
    lesson: 8,
  },
  {
    id: 187,
    word: "旁",
    pinyin: "páng",
    english: "beside, next to",
    vietnamese: "bên cạnh",
    example: "我在旁边等你。 (I am waiting for you beside the table.)",
    lesson: 8,
  },
  {
    id: 188,
    word: "呢",
    pinyin: "ne",
    english: "particle indicating a question",
    vietnamese: "bộ phận hỏi",
    example: "你呢？ (What about you?)",
    lesson: 8,
  },
  {
    id: 189,
    word: "下面/下边",
    pinyin: "xiàmiàn/xiàbiān",
    english: "below, underneath",
    vietnamese: "phía dưới",
    example: "书在桌子下面。 (The book is under the table.)",
    lesson: 8,
  },
  {
    id: 190,
    word: "下",
    pinyin: "xià",
    english: "below, underneath",
    vietnamese: "dưới",
    example: "书在桌子下面。 (The book is under the table.)",
    lesson: 8,
  },
  {
    id: 191,
    word: "中间",
    pinyin: "zhōngjiān",
    english: "middle, between, among",
    vietnamese: "giữa",
    example: "我在中间等你。 (I am waiting for you in the middle.)",
    lesson: 8,
  },
  {
    id: 192,
    word: "坐",
    pinyin: "zuò",
    english: "to sit",
    vietnamese: "ngồi",
    example: "我坐在椅子上。 (I am sitting on the chair.)",
    lesson: 9,
  },
  {
    id: 193,
    word: "喝",
    pinyin: "hē",
    english: "to drink",
    vietnamese: "uống",
    example: "我喝一杯水。 (I am drinking a cup of water.)",
    lesson: 9,
  },
  {
    id: 194,
    word: "茶",
    pinyin: "chá",
    english: "tea",
    vietnamese: "trà",
    example: "我喝一杯茶。 (I am drinking a cup of tea.)",
    lesson: 9,
  },
  {
    id: 195,
    word: "还是",
    pinyin: "háishi",
    english: "still, yet (used to indicate a preference or choice)",
    vietnamese: "vẫn",
    example: "我还是喜欢喝茶。 (I still like to drink tea.)",
    lesson: 9,
  },
  {
    id: 196,
    word: "咖啡",
    pinyin: "kāfēi",
    english: "coffee",
    vietnamese: "cà phê",
    example: "我喝一杯咖啡。 (I am drinking a cup of coffee.)",
    lesson: 9,
  },
  {
    id: 197,
    word: "吃",
    pinyin: "chī",
    english: "to eat",
    vietnamese: "ăn",
    example: "我吃一个苹果。 (I am eating an apple.)",
    lesson: 9,
  },
  {
    id: 198,
    word: "面包",
    pinyin: "miànbāo",
    english: "bread",
    vietnamese: "bánh mì",
    example: "我吃一个面包。 (I am eating a piece of bread.)",
    lesson: 9,
  },
  {
    id: 199,
    word: "欢迎",
    pinyin: "huānyíng",
    english: "welcome",
    vietnamese: "chào mừng",
    example: "欢迎来到中国。 (Welcome to China.)",
    lesson: 9,
  },
  {
    id: 200,
    word: "位",
    pinyin: "wei",
    english: "a measure word for people (polite)",
    vietnamese: "vị",
    example: "欢迎位客人。 (Welcome, guests.)",
    lesson: 9,
  },
  {
    id: 201,
    word: "(一)点儿",
    pinyin: "(yì) diǎnr",
    english: "a little, a bit",
    vietnamese: "một chút",
    example: "我有一点儿饿。 (I am a little hungry.)",
    lesson: 9,
  },
  {
    id: 202,
    word: "这儿",
    pinyin: "zhèr",
    english: "here",
    vietnamese: "đây",
    example: "我在这儿等你。 (I am waiting for you here.)",
    lesson: 9,
  },
  {
    id: 203,
    word: "米饭",
    pinyin: "mǐfàn",
    english: "rice",
    vietnamese: "cơm",
    example: "我吃米饭。 (I am eating rice.)",
    lesson: 9,
  },
  {
    id: 204,
    word: "饺子",
    pinyin: "jiǎozi",
    english: "dumplings",
    vietnamese: "há cảo",
    example: "我吃饺子。 (I am eating dumplings.)",
    lesson: 9,
  },
  {
    id: 205,
    word: "包子",
    pinyin: "bāozi",
    english: "steamed bun",
    vietnamese: "bánh bao",
    example: "我吃包子。 (I am eating a steamed bun.)",
    lesson: 9,
  },
  {
    id: 206,
    word: "面条",
    pinyin: "miàntiáo",
    english: "noodles",
    vietnamese: "mỳ",
    example: "我吃面条。 (I am eating noodles.)",
    lesson: 9,
  },
  {
    id: 207,
    word: "咱们",
    pinyin: "zánmen",
    english: "we, us (informal)",
    vietnamese: "chúng ta",
    example: "咱们一起去吧。 (Let's go together.)",
    lesson: 9,
  },
  {
    id: 208,
    word: "吧",
    pinyin: "ba",
    english: "a modal particle indicating suggestion or assumption",
    vietnamese:
      "đi, nhé, thôi, nào, ... (một trợ từ biểu thị đề nghị hoặc giả định)",
    example: "咱们一起去吧。 (Let's go together.)",
    lesson: 9,
  },
  {
    id: 209,
    word: "行",
    pinyin: "xíng",
    english: "to be okay, to be all right",
    vietnamese: "được, ổn",
    example: "好的，我们走吧。 (Okay, let's go.)",
    lesson: 9,
  },
  {
    id: 210,
    word: "不行",
    pinyin: "bù xíng",
    english: "not okay, not all right",
    vietnamese: "không được, không ổn",
    example: "不行，我不能去。 (No, I can't go.)",
    lesson: 9,
  },
  {
    id: 211,
    word: "啤酒",
    pinyin: "píjiǔ",
    english: "beer",
    vietnamese: "bia",
    example: "我喝一杯啤酒。 (I am drinking a cup of beer.)",
    lesson: 9,
  },
  {
    id: 212,
    word: "酒",
    pinyin: "jiǔ",
    english: "alcohol, wine",
    vietnamese: "rượu",
    example: "我喝一杯酒。 (I am drinking a cup of wine.)",
    lesson: 9,
  },
  {
    id: 213,
    word: "苹果",
    pinyin: "píngguǒ",
    english: "apple",
    vietnamese: "táo",
    example: "我吃一个苹果。 (I am eating an apple.)",
    lesson: 10,
  },
  {
    id: 214,
    word: "快/元",
    pinyin: "kuài/yuán",
    english:
      "units of currency in China (kuài is more colloquial, yuán is more formal)",
    vietnamese:
      "đơn vị tiền tệ ở Trung Quốc (kuài là cách nói thông thường, yuán là cách nói trang trọng hơn)",
    example: "这个苹果多少钱？ (How much is this apple?)",
    lesson: 10,
  },
  {
    id: 215,
    word: "毛/角",
    pinyin: "máo/jiǎo",
    english:
      "units of currency in China (máo is the smallest unit, jiǎo is a larger unit)",
    vietnamese:
      "đơn vị tiền tệ ở Trung Quốc (máo là đơn vị nhỏ nhất, jiǎo là đơn vị lớn hơn)",
    example: "这个苹果多少钱？ (How much is this apple?)",
    lesson: 10,
  },
  {
    id: 216,
    word: "分",
    pinyin: "fēn",
    english: "the smallest unit of currency in China",
    vietnamese: "phân",
    example: "这个苹果多少钱？ (How much is this apple?)",
    lesson: 10,
  },
  {
    id: 217,
    word: "斤",
    pinyin: "jīn",
    english: "a unit of weight in China",
    vietnamese: "kiện",
    example: "这个苹果有半斤重。 (This apple weighs half a jin.)",
    lesson: 10,
  },
  {
    id: 218,
    word: "香蕉",
    pinyin: "xiāngjiāo",
    english: "banana",
    vietnamese: "chuối",
    example: "我吃一根香蕉。 (I am eating a banana.)",
    lesson: 10,
  },

  {
    id: 219,
    word: "要",
    pinyin: "yào",
    english: "to want, to need",
    vietnamese: "muốn, cần",
    example: "我要喝水。 (I want to drink water.)",
    lesson: 10,
  },
  {
    id: 220,
    word: "一共",
    pinyin: "yígòng",
    english: "in total, all together",
    vietnamese: "tất cả",
    example: "我们一共吃了三碗饭。 (We ate three bowls of rice in total.)",
    lesson: 10,
  },
  {
    id: 221,
    word: "钱",
    pinyin: "qián",
    english: "money",
    vietnamese: "tiền",
    example: "我有好多钱。 (I have a lot of money.)",
    lesson: 10,
  },
  {
    id: 222,
    word: "种",
    pinyin: "zhǒng",
    english: "kind, type (a measure word for types or kinds of things)",
    vietnamese: "loại, chủng loại",
    example: "这是什么种水果？ (What kind of fruit is this?)",
    lesson: 10,
  },
  {
    id: 223,
    word: "太...了",
    pinyin: "tài...le",
    english: "too ... (used to express that something is excessive)",
    vietnamese: "quá ... (được sử dụng để biểu thị rằng điều gì đó là quá mức)",
    example: "这太贵了。 (This is too expensive.)",
    lesson: 10,
  },
  {
    id: 224,
    word: "贵",
    pinyin: "guì",
    english: "expensive",
    vietnamese: "đắt",
    example: "这太贵了。 (This is too expensive.)",
    lesson: 10,
  },
  {
    id: 225,
    word: "便宜",
    pinyin: "piányi",
    english: "cheap",
    vietnamese: "rẻ",
    example: "这很便宜。 (This is cheap.)",
    lesson: 10,
  },
  {
    id: 226,
    word: "怎么样",
    pinyin: "zěnme yàng",
    english: "how is it, what about it",
    vietnamese: "thế nào",
    example: "这怎么样？ (How is this?)",
    lesson: 10,
  },
  {
    id: 227,
    word: "还",
    pinyin: "hái",
    english: "in addition",
    vietnamese: "còn, thêm vào đó",
    example: "这还不够。 (This is still not enough.)",
    lesson: 10,
  },
  {
    id: 228,
    word: "别的",
    pinyin: "biéde",
    english: "other",
    vietnamese: "khác",
    example: "别的地方 (elsewhere)",
    lesson: 10,
  },
  {
    id: 229,
    word: "给",
    pinyin: "gěi",
    english: "to give",
    vietnamese: "cho",
    example: "给我一本书。 (Give me a book.)",
    lesson: 10,
  },
  {
    id: 230,
    word: "找",
    pinyin: "zhǎo",
    english: "to give change",
    vietnamese: "tìm lại tiền, thối tiền",
    example: "我要找零钱。 (I want some change.)",
    lesson: 10,
  },
  {
    id: 231,
    word: "好久不见",
    pinyin: "hǎo jiǔ bù jiàn",
    english: "long time no see",
    vietnamese: "rất lâu không gặp",
    example: "好久不见，你最近怎么样？ (Long time no see, how have you been?)",
    lesson: 11,
  },
  {
    id: 232,
    word: "好久",
    pinyin: "hǎo jiǔ",
    english: "for a long time",
    vietnamese: "rất lâu",
    example: "好久不见！ (Long time no see!)",
    lesson: 11,
  },
  {
    id: 233,
    word: "久",
    pinyin: "jiǔ",
    english: "long time",
    vietnamese: "lâu",
    example: "好久不见！ (Long time no see!)",
    lesson: 11,
  },
  {
    id: 234,
    word: "见",
    pinyin: "jiàn",
    english: "to see",
    vietnamese: "gặp",
    example: "好久不见！ (Long time no see!)",
    lesson: 11,
  },
  {
    id: 235,
    word: "忙",
    pinyin: "máng",
    english: "busy",
    vietnamese: "bận",
    example: "我很忙。 (I am very busy.)",
    lesson: 11,
  },
  {
    id: 236,
    word: "身体",
    pinyin: "shēntǐ",
    english: "body, health",
    vietnamese: "thân thể, sức khỏe",
    example: "我的身体很好。 (My body is very healthy.)",
    lesson: 11,
  },
  {
    id: 237,
    word: "去",
    pinyin: "qù",
    english: "to go",
    vietnamese: "đi",
    example: "我去商店。 (I am going to the store.)",
    lesson: 11,
  },
  {
    id: 238,
    word: "回",
    pinyin: "huí",
    english: "to return, to go back",
    vietnamese: "trở lại, quay lại",
    example: "我要回学校。 (I want to go back to school.)",
    lesson: 11,
  },
  {
    id: 239,
    word: "换",
    pinyin: "huàn",
    english: "to change, to exchange",
    vietnamese: "đổi",
    example: "我要换钱。 (I want to exchange money.)",
    lesson: 11,
  },
  {
    id: 240,
    word: "还",
    pinyin: "hái",
    english: "still, yet",
    vietnamese: "vẫn, còn",
    example: "他还在这里。 (He is still here.)",
    lesson: 11,
  },
  {
    id: 241,
    word: "住",
    pinyin: "zhù",
    english: "to live, to stay",
    vietnamese: "ở",
    example: "我住在学校。 (I live at the school.)",
    lesson: 11,
  },
  {
    id: 242,
    word: "电话",
    pinyin: "diànhuà",
    english: "telephone",
    vietnamese: "điện thoại",
    example: "这是我的电话。 (This is my phone.)",
    lesson: 11,
  },
  {
    id: 243,
    word: "号码",
    pinyin: "hàomǎ",
    english: "number",
    vietnamese: "số",
    example: "这是我的电话号码。 (This is my phone number.)",
    lesson: 11,
  },
  {
    id: 244,
    word: "空儿",
    pinyin: "kòngr",
    english: "free time",
    vietnamese: "thời gian rảnh",
    example: "你有空儿吗？ (Do you have free time?)",
    lesson: 11,
  },
  {
    id: 245,
    word: "来",
    pinyin: "lái",
    english: "to come",
    vietnamese: "đến",
    example: "他来了。 (He came.)",
    lesson: 11,
  },
  {
    id: 246,
    word: "玩儿",
    pinyin: "wánr",
    english: "to play",
    vietnamese: "chơi",
    example: "我们去玩儿吧。 (Let's go play.)",
    lesson: 11,
  },
  {
    id: 247,
    word: "再见",
    pinyin: "zài jiàn",
    english: "goodbye",
    vietnamese: "tạm biệt",
    example: "再见！ (Goodbye!)",
    lesson: 11,
  },
  {
    id: 248,
    word: "百",
    pinyin: "bǎi",
    english: "hundred",
    vietnamese: "trăm",
    example: "一百 (one hundred)",
    lesson: 11,
  },
  {
    id: 249,
    word: "千",
    pinyin: "qiān",
    english: "thousand",
    vietnamese: "ngàn",
    example: "一千 (one thousand)",
    lesson: 11,
  },
  {
    id: 250,
    word: "万",
    pinyin: "wàn",
    english: "ten thousand",
    vietnamese: "vạn",
    example: "一万 (ten thousand)",
    lesson: 11,
  },
  {
    id: 251,
    word: "美元",
    pinyin: "měiyuán",
    english: "US dollar",
    vietnamese: "đô la Mỹ",
    example: "一百美元 (one hundred US dollars)",
    lesson: 11,
  },
  {
    id: 252,
    word: "欧元",
    pinyin: "ǒuyuán",
    english: "Euro",
    vietnamese: "euro",
    example: "一百欧元 (one hundred Euros)",
    lesson: 11,
  },
  {
    id: 253,
    word: "日元",
    pinyin: "rìyuán",
    english: "Japanese yen",
    vietnamese: "yên Nhật",
    example: "一百日元 (one hundred Japanese yen)",
    lesson: 11,
  },
  {
    id: 254,
    word: "人民币",
    pinyin: "rénmínbì",
    english: "Chinese yuan",
    vietnamese: "nhân dân tệ Trung Quốc",
    example: "一百人民币 (one hundred Chinese yuan)",
    lesson: 11,
  },
  {
    id: 255,
    word: "人民",
    pinyin: "rénmín",
    english: "people",
    vietnamese: "người dân",
    example: "中国人民 (Chinese people)",
    lesson: 11,
  },
  {
    id: 256,
    word: "币",
    pinyin: "bì",
    english: "currency",
    vietnamese: "tiền tệ",
    example: "美元 (US dollar)",
    lesson: 11,
  },
  {
    id: 257,
    word: "超市",
    pinyin: "chāoshì",
    english: "supermarket",
    vietnamese: "siêu thị",
    example: "我去超市买东西。 (I'm going to the supermarket to buy things.)",
    lesson: 11,
  },
  {
    id: 258,
    word: "买",
    pinyin: "mǎi",
    english: "to buy",
    vietnamese: "mua",
    example: "我买了一本书。 (I bought a book.)",
    lesson: 11,
  },
  {
    id: 259,
    word: "东西",
    pinyin: "dōngxi",
    english: "thing, item",
    vietnamese: "đồ vật",
    example: "这是什么东西？ (What is this thing?)",
    lesson: 11,
  },
  {
    id: 260,
    word: "每",
    pinyin: "měi",
    english: "every, each",
    vietnamese: "mỗi",
    example: "我每天都吃早餐。 (I eat breakfast every day.)",
    lesson: 12,
  },
  {
    id: 261,
    word: "骑",
    pinyin: "qí",
    english: "to ride",
    vietnamese: "cưỡi, đạp (xe đạp)",
    example: "我喜欢骑自行车。 (I like riding a bicycle.)",
    lesson: 12,
  },
  {
    id: 262,
    word: "自行车",
    pinyin: "zìxíngchē",
    english: "bicycle",
    vietnamese: "xe đạp",
    example: "我喜欢骑自行车。 (I like riding a bicycle.)",
    lesson: 12,
  },
  {
    id: 263,
    word: "教室",
    pinyin: "jiàoshì",
    english: "classroom",
    vietnamese: "lớp học",
    example: "我们在教室里。 (We are in the classroom.)",
    lesson: 12,
  },
  {
    id: 264,
    word: "走路",
    pinyin: "zǒu lù",
    english: "to walk",
    vietnamese: "đi bộ",
    example: "我喜欢走路。 (I like walking.)",
    lesson: 12,
  },
  {
    id: 265,
    word: "走",
    pinyin: "zǒu",
    english: "to walk",
    vietnamese: "đi",
    example: "我走过去。 (I walked over.)",
    lesson: 12,
  },
  {
    id: 266,
    word: "路",
    pinyin: "lù",
    english: "road, path",
    vietnamese: "đường",
    example: "这条路很宽。 (This road is very wide.)",
    lesson: 12,
  },
  {
    id: 267,
    word: "坐",
    pinyin: "zuò",
    english: "to travel by",
    vietnamese: "ngồi",
    example: "我坐公交车去学校。 (I take the bus to school.)",
    lesson: 12,
  },
  {
    id: 268,
    word: "出租车",
    pinyin: "chūzūchē",
    english: "taxi",
    vietnamese: "xe taxi",
    example: "我叫了一辆出租车。 (I called a taxi.)",
    lesson: 12,
  },
  {
    id: 269,
    word: "出租",
    pinyin: "chūzū",
    english: "to rent",
    vietnamese: "cho thuê",
    example: "我租了一辆车。 (I rented a car.)",
    lesson: 12,
  },
  {
    id: 270,
    word: "车",
    pinyin: "chē",
    english: "car",
    vietnamese: "xe",
    example: "我有一辆车。 (I have a car.)",
    lesson: 12,
  },
  {
    id: 271,
    word: "周末",
    pinyin: "zhōumò",
    english: "weekend",
    vietnamese: "cuối tuần",
    example: "我们周末去公园。 (We go to the park on the weekend.)",
    lesson: 12,
  },
  {
    id: 272,
    word: "公共汽车",
    pinyin: "gōnggòng qìchē",
    english: "bus",
    vietnamese: "xe buýt",
    example: "我坐公共汽车去学校。 (I take the bus to school.)",
    lesson: 12,
  },
  {
    id: 273,
    word: "公共",
    pinyin: "gōnggòng",
    english: "public",
    vietnamese: "công cộng",
    example: "公共设施 (public facilities)",
    lesson: 12,
  },
  {
    id: 274,
    word: "汽车",
    pinyin: "qìchē",
    english: "car",
    vietnamese: "xe",
    example: "我有一辆车。 (I have a car.)",
    lesson: 12,
  },
  {
    id: 275,
    word: "怎么",
    pinyin: "zěnme",
    english: "how",
    vietnamese: "thế nào",
    example: "你怎么了？ (What's wrong with you?)",
    lesson: 12,
  },
  {
    id: 276,
    word: "挤",
    pinyin: "jǐ",
    english: "crowded",
    vietnamese: "đông đúc",
    example: "公共汽车很挤。 (The bus is very crowded.)",
    lesson: 12,
  },
  {
    id: 277,
    word: "打的",
    pinyin: "dǎ dī",
    english: "to take a taxi",
    vietnamese: "đi taxi",
    example: "我打的去机场。 (I take a taxi to the airport.)",
    lesson: 12,
  },
  {
    id: 278,
    word: "辆",
    pinyin: "liàng",
    english: "a measure word for vehicles",
    vietnamese: "chiếc",
    example: "我有一辆自行车。 (I have a bicycle.)",
    lesson: 12,
  },
  {
    id: 279,
    word: "借",
    pinyin: "jiè",
    english: "to borrow, to lend",
    vietnamese: "mượn",
    example: "我可以借给你这本书。 (I can lend you this book.)",
    lesson: 12,
  },
  {
    id: 280,
    word: "出发",
    pinyin: "chūfā",
    english: "to start off, to set out, to depart",
    vietnamese: "khởi hành",
    example: "我们出发去旅行。 (We set out for a trip.)",
    lesson: 12,
  },
  {
    id: 281,
    word: "没问题",
    pinyin: "méi wèntí",
    english: "no problem",
    vietnamese: "không vấn đề gì",
    example: "没问题。 (No problem.)",
    lesson: 12,
  },
  {
    id: 282,
    word: "问题",
    pinyin: "wèntí",
    english: "problem",
    vietnamese: "vấn đề",
    example: "有什么问题吗？ (Is there a problem?)",
    lesson: 12,
  },
  {
    id: 283,
    word: "找",
    pinyin: "zhao",
    english: "to look for, to find",
    vietnamese: "tìm",
    example: "我在找我的钥匙。 (I am looking for my keys.)",
    lesson: 12,
  },
];
